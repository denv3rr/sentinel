...
import type { OperatingMode, SettingsResponse } from "../lib/types";
...
export default function SettingsPage({ settings, onRefresh }: Props) {
  ...
  const [operatingMode, setOperatingMode] = useState<OperatingMode>(settings.operating_mode);
  ...
  useEffect(() => {
    ...
    setOperatingMode(settings.operating_mode);
  ...
  return (
    <section className="space-y-4">
      <article className="card rounded-xl p-4 shadow-panel">
        <h3 className="mb-2 text-sm font-semibold">Operating Mode</h3>
        <p className="mb-3 text-xs opacity-70">Use these presets for everyday use. Sentinel adjusts arming behavior automatically.</p>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_auto]">
          <label>
            <span className="mb-1 block text-xs opacity-70">Mode</span>
            <select
              value={operatingMode}
          ...
          <button
            ...
            onClick={async () => {
              ...
              setError("");
              try {
                await api.setOperatingMode(operatingMode);
                setNotice(`Operating mode updated to ${operatingMode}`);
                await onRefresh();
              } catch (e) {
                setError((e as Error).message || "Failed to save operating mode");
              }
            }}
          >
            Save mode
      ...
      <article className="card rounded-xl p-4 shadow-panel">
        ...
        <p className="mb-2 text-xs opacity-70">Most users can rely on Operating Mode. Use this for manual override.</p>
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setError((e as Error).message || "Failed to update arm state");
            }
          }}
        >
          Save arm state
      ...
      <details className="card rounded-xl p-4 shadow-panel">
        <summary className="cursor-pointer text-sm font-semibold">Advanced Storage Settings</summary>
        <div className="mt-3 space-y-4">
          ...
          <article className="rounded-xl border border-slate-400/30 p-3">
      ...
      <details className="card rounded-xl p-4 shadow-panel">
        <summary className="cursor-pointer text-sm font-semibold">Advanced Detection and Network Settings</summary>
        <div className="mt-3 space-y-4">
          <article className="rounded-xl border border-slate-400/30 p-4">
